...
class MercurializeFilter: CIFilter {
    ...
    override var outputImage: CIImage!
    {
        ...
        if sphereImage == nil
        {
            ...
            omniLightNode.position.x = CGFloat(Float(-50 + (inputLightPosition.x * 100)))
            omniLightNode.position.y = CGFloat(Float(-50 + (inputLightPosition.y * 100)))
            
            ambientLightNode.color = inputAmbientLightColor
            
//            sceneKitView.prepareObject(sceneKitView.scene!, shouldAbortBlock: {false})
            guard let shot = sceneKitView.snapshot().tiffRepresentation,
            let imageBitmap = NSBitmapImageRep(data:shot),
            let coreImage = CIImage(bitmapImageRep: imageBitmap) else {
                print("⚠️ Error taking snapshot of the Sceneview")
                return CIImage()
            }
            
            //let cgimg = shot.cgImage(forProposedRect: NSRect(origin: .zero, size: shot.size), context: nil, hints: nil)
            
            sphereImage = coreImage
        }
    ...
    func setUpSceneKit()
    {
        ...
        sceneKitView.backgroundColor = NSColor.black
